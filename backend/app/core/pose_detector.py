"""姿势检测模块（移植自 SpriteFrameStudio）。

支持两种检测器：
- MediaPipe Pose（detect_pose）
- RTMPose Wholebody / rtmlib（detect_pose_rtm）

模型目录通过 app.config 解析（SPRITE_MODELS_DIR/rtmpose）。
"""
...
from app.models.pose_data import PoseData as _PoseData


def get_default_rtm_models():
    """获取默认的 RTMPose 模型路径（从配置的 models/rtmpose 目录）。

    Returns:
        tuple: (检测模型路径, 姿态模型路径) 或 (None, None)
    """
    settings = get_settings()
    models_dir = settings.resolved_models_dir / "rtmpose"
...
class RegionalFeatureData:
    ...
    def __init__(self, frame_index: int,
        ...
        self.id = str(uuid.uuid4())
        self.frame_index = frame_index
        self.upper_gray = upper_gray
        self.middle_gray = middle_gray
        self.lower_gray = lower_gray
...
class PoseDetector:
    ...
    def _init_mediapipe(self):
        """初始化 MediaPipe solutions(懒加载)。

        新版 mediapipe(>=1.0) 已移除 mp.solutions 旧 API，此时返回 False，
        由 detect_pose 自动回退到 RTMPose(rtmlib)。
        """
        if self._pose is None:
            try:
                import mediapipe as mp
                if not hasattr(mp, "solutions"):
                    print("[WARNING] mediapipe>=1.0 已移除 solutions API，pose 模式将回退到 RTMPose")
                    return False
    ...
    def _mediapipe_available(self) -> bool:
        """mediapipe solutions API 是否可用。"""
        try:
            import mediapipe as mp
    ...
    def _detect_pose_mediapipe(self, image: np.ndarray, frame_index: int = 0):
        """使用 MediaPipe solutions 检测姿势。"""
        from app.models.pose_data import PoseData, Landmark
    ...
    def _init_rtm_model(self):
        ...
        if self._rtm_model is None:
            try:
                # 优先尝试本地 rtmlib 目录（若项目根存在 rtmlib/），否则使用 pip 包
                import sys
    ...
    def detect_pose(self, image: np.ndarray, frame_index: int = 0):
        ...
        优先 MediaPipe solutions；新版 mediapipe 无 solutions API 时自动
        回退到 RTMPose(rtmlib，离线模型)。两种结果均为 PoseData。
        """
        if self._mediapipe_available():
            try:
                result = self._detect_pose_mediapipe(image, frame_index)
                if result is not None:
                    return result
            except Exception as e:
                print(f"[WARNING] MediaPipe 检测失败，回退 RTMPose: {e}")
        return self.detect_pose_rtm(image, frame_index)
    ...
    def draw_pose_on_image(
        ...
        landmark_color: Tuple[int, int, int] = (0, 184, 212),
        ...
        if draw_connections:
            for start_idx, end_idx in _PoseData.POSE_CONNECTIONS:
    ...
    def batch_extract_contours(
        ...
        for i, (image, frame_index) in enumerate(images):
            ...
            results.append(self.extract_contour(image, frame_index))
    ...
    def batch_extract_image_features(
        ...
        for i, (image, frame_index) in enumerate(images):
            ...
            results.append(self.extract_image_features(image, frame_index))
    ...
    def batch_extract_regional_features(
        ...
        for i, (image, frame_index) in enumerate(images):
            ...
            results.append(self.extract_regional_features(image, frame_index, weights))
