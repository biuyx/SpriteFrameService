...
- **内置使用手册**：顶栏「? 手册」打开，按工序讲解，含概念入门、进阶与排错；从某道工序打开会定位到对应章节。
...
**动画名**默认从参考视频库取：每条模板有「Spine 动画名」字段，可逐条编辑，
也可按变体名一键套用建议值（走路→walk1、工作→sit_work1…）。

导出弹窗的动作表里每行都能**直接改名**，只作用于这一次导出；勾上「记住改名」
就记到动作上（`spine_anim`），之后手动导出与一键流水线的自动收口都用它。
改回推断值会自动取消记住，重新由模板说了算。选了导出模板时，输入框会列出
模板里已有的动画名——挑一个即把该动作接到那个动画上，填模板里没有的名字会
标黄提示将作为新动画导出。重名会互相覆盖，前后端都拦，并指出是哪两个动作撞了。
